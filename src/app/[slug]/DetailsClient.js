...
import {
  ...
  Battery,
  Smartphone,
  BatteryCharging, ChevronRight, ChevronDown, Heart, Wifi, Search, Filter,
  Music,
  Fingerprint, Calendar, MonitorSmartphone, Ruler, Bot, Info,
...
export default function Details({ phoneDetails, normalizedSpecs }) {
  ...
  useEffect(() => {
    ...
    Object.entries(normalizedSpecs).forEach(([section, details]) => {
      ...
      Object.entries(details).forEach(([k, v]) => {
        ...
        // Apply search
        if (searchTerm === '' || searchValue.includes(searchTerm.toLowerCase())) {
          filteredDetails[k] = v;
        }
      });

      if ((selectedValues.length === 0 || selectedValues.includes(section)) && Object.keys(filteredDetails).length > 0) {
  ...
  const images = selectedColorData?.images?.length ? selectedColorData.images : [];
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-gray-500">The requested device could not be found.</p>
        </div>
      </div>
    );
  return (
          ...
          <div className="lg:col-span-7 flex flex-col space-y-8">
            {/* --- Top Highlights Section --- */}
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <SpecCard icon={<Calendar size={18} className="text-blue-600" />} label="Release Date" value={phoneDetails.release_date || "2025"} />
                <SpecCard icon={<Ruler size={18} className="text-indigo-600" />} label="Dimensions" value={phoneDetails?.searchIndex?.dimensions || "190g, 7.4mm thickness"} />
                <SpecCard icon={<MonitorSmartphone size={18} className="text-teal-600" />} label="Display" value={`${phoneDetails?.searchIndex?.screen_size_inches || "6.8"}" ${phoneDetails?.searchIndex?.display_type || "AMOLED"}, ${phoneDetails?.searchIndex?.refresh_rate_max || "120"}Hz`} subValue="1256x2760 pixels" />
                <SpecCard icon={<Cpu size={18} className="text-orange-600" />} label="Processor" value={phoneDetails?.searchIndex?.chipset || "Snapdragon 8 Gen 3"} />
                <SpecCard icon={<Camera size={18} className="text-indigo-600" />} label="Camera" value={`${phoneDetails?.searchIndex?.main_camera_mp || "200MP"} Triple Camera`} />
                <SpecCard icon={<Battery size={18} className="text-emerald-600" />} label="Battery" value={`${phoneDetails?.searchIndex?.battery_capacity_mah || 5000}mAh`} />
                <SpecCard icon={<BatteryCharging size={18} className="text-yellow-600" />} label="Charging" value={phoneDetails?.searchIndex?.wired_charging || "45W Fast Charging"} />
                <SpecCard icon={<Bot size={18} className="text-rose-600" />} label="OS" value={phoneDetails.os || "Android 15"} subValue="7 years updates" />
              </div>
                    ...
                    >
                      <div>
                        <p className="text-base font-semibold text-gray-900">
                          {variant.ram && variant.storage ? `${variant.ram}GB / ${variant.storage}GB` : 'Standard'}
                        </p>
          ...
          <div className="p-8 sm:p-8">
            ...
            <div className="flex flex-col items-start gap-2">
              ...
              <div className="relative w-full flex items-center gap-3">
                ...
                {Object.keys(filteredSpecs).length > 0 && (
                  ...
                  >
                    {expandedSections.size === Object.keys(filteredSpecs).length ? (
                      <>
                ...
                <Select
                  ...
                  styles={{
                    control: (base, state) => ({
                      ...
                      paddingLeft: "3rem",
                      border: "2px solid",
                      borderColor: state.isFocused ? "#60a5fa" : "rgba(226, 232, 240, 0.6)",
                      backgroundColor: state.isFocused ? "#ffffff" : "rgba(255, 255, 255, 0.8)",
            ...
            <div className="space-y-4 mt-3">
              {Object.entries(filteredSpecs).map(([section, details]) => {
                const countNonEmptyKeys = (obj) => {
                  return Object.keys(obj).filter(key => obj[key] !== null && obj[key] !== '').length;
                };
                      ...
                      <div className="border-t border-gray-100 bg-gray-50/50">
                        <div className="p-4 space-y-3">
                          {Object.entries(details).map(([key, value]) => {
                            ...
                            // Shared classes
                            const containerClass = "flex flex-col gap-3 p-3 bg-white rounded-lg border border-gray-100 hover:border-gray-200 transition-colors duration-200";
                            const labelClass = "text-sm font-medium text-gray-700 capitalize";
                            const tableClass = "min-w-full text-sm text-left border border-gray-200 rounded-md";
                            const cellClass = "px-3 py-2 text-gray-800 border-b border-gray-200";
                            const headerClass = "px-3 py-2 font-medium text-gray-700 capitalize border-b border-gray-200";
                            ...
                            const flexDirection = !isArray && !isNumericKey ? 'sm:flex-row' : '';
                            const labelWidth = isArray ? 'w-full mb-2' : 'sm:w-1/3';
                            const contentWidth = isArray || isNumericKey ? 'w-full overflow-x-auto' : 'sm:w-2/3';
                            if (value !== null) {
                              return (
                                <div key={key} className={`${containerClass} ${flexDirection}`}>
                                  ...
                                  <div className={contentWidth}>
                                    {isArray ? (
                                      <table className={tableClass}>
                                        <tbody>
                                          {Array.from({ length: Math.ceil(value.length / 2) }, (_, rowIndex) => (
            ...
            {Object.keys(filteredSpecs).length === 0 && (
              <div className="text-center py-16">
                ...
                <h3 className="text-2xl font-bold text-gray-600 mb-3">No specifications found</h3>
                <p className="text-gray-500 text-lg mb-6">Try adjusting your search terms or filter selection</p>
                <button
                  onClick={() => {
                    setSearchTerm('');
